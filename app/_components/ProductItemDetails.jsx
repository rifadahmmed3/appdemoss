...
function ProductItemDetails({product}) {
        ...
        setLoading(true)
            ...
            const data={
                data:{
                    ...
                    amount:parseFloat(quantity*productTotalPrice),
                    products:product.id,
                    users_permissions_users:user.id,
                    userId:user.id,
                    userPoint:parseFloat(quantity*productTotalPoint)
                }
  ...
  return (
    <div className='grid grid-cols-1 sm:flex-row md:grid-cols-2 p-5 bg-white text-black object-contain'>
        ...
        <div className='flex flex-col gap-3 sm:flex-row mt-5'>
            {/* <h2 className='font-bold text-xl'>{product.attributes.name}</h2> */}
            ...
            <div className='flex flex-col items-baseline gap-3'>
                ...
                <h2 className='font-bold text-xl'>Price: {(quantity*productTotalPrice).toFixed(2)}৳</h2>
                <h2 className='font-medium'> Point: {(quantity*productTotalPoint).toFixed(2)}</h2>
                <Button disabled={loading} className="flex gap-3" onClick={()=>addToCart()}>
                    <ShoppingBag />
                    {loading? <LoaderCircle className='animate-spin'/>:'Add To Cart'}
                </Button>
                {/* <h2><span className='font-bold'>Category:</span> {product.attributes.categories.data.attributes.Name}</h2> */}
            </div>
        </div>
    </div>
  )
